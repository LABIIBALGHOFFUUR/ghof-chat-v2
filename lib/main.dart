...
class _ChatRoomScreenState extends State<ChatRoomScreen> {
  ...
  final List<ChatMessage> _messages = [
    ChatMessage(text: 'Halo! Ini chat dummy (lokal).', isMe: false, time: DateTime.now()),
    ChatMessage(text: 'Siap, nanti kita sambung ke server realtime.', isMe: true, time: DateTime.now()),
  ];
  ...
  void _sendText() {
    ...
    setState(() {
      _messages.add(ChatMessage(text: t, isMe: true, time: DateTime.now()));
      _textCtrl.clear();
    });

    // auto scroll setelah frame render
    WidgetsBinding.instance.addPostFrameCallback((_) => _scrollToBottom());
  }

  void _voiceNotePlaceholder() {
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(content: Text('Voice note: belum aktif (nanti tahap berikutnya).')),
    );
  }

  void _callPlaceholder(String which) {
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(content: Text('$which: belum aktif (nanti pakai WebRTC).')),
    );
  }

  void _menuSelected(String v) {
    if (v == 'block') {
      showDialog(
        context: context,
        builder: (_) => AlertDialog(
          title: const Text('Blokir pengguna?'),
          content: const Text('Fitur block akan aktif setelah backend + privacy dibuat.'),
          actions: [
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(content: Text('Menu "$v" (placeholder)')),
    );
  }
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        actions: [
          ...
          IconButton(onPressed: () => _callPlaceholder('Video call'), icon: const Icon(Icons.videocam_outlined)),
          IconButton(onPressed: () => _callPlaceholder('Voice call'), icon: const Icon(Icons.call_outlined)),
          PopupMenuButton<String>(
            onSelected: _menuSelected,
            itemBuilder: (context) => const [
              PopupMenuItem(value: 'view_contact', child: Text('Lihat profil')),
              PopupMenuItem(value: 'mute', child: Text('Bisukan')),
            ...
            child: Row(
              children: [
                ...
                CircleAvatar(
                  backgroundColor: ghofGreen,
                  child: IconButton(
                    onPressed: hasText ? _sendText : _voiceNotePlaceholder,
...
class _MessageBubble extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final bubbleColor = message.isMe ? const Color(0xFFD7FAD1) : Colors.white;
          ...
          child: Column(
            ...
            children: [
              ...
              Text(
                ...
                style: TextStyle(fontSize: 11, color: Colors.black.withAlpha(140)),
              ),
            ],
          ),
        ),
